...
export default function JsonTreeEditor({ jsonText, onChange, height = '300px' }: Props) {
	...
	useEffect(() => {
		...
		try {
			...
			// fallback to empty object if initial parse fails
	...
	// keep editor in sync if jsonText changes externally
	useEffect(() => {
		if (!editorRef.current) return;
		try {
			const current = editorRef.current.get();
			const next = JSON.parse(jsonText || '{}');
			// shallow compare to avoid unnecessary set
			if (JSON.stringify(current) !== JSON.stringify(next)) {
				editorRef.current.set(next);
			}
		} catch {
			// ignore if external text is invalid
		}
	}, [jsonText]);
